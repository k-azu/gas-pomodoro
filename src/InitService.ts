function initializeSpreadsheet(): void {
  ...
  if (!logSheet) {
    ...
    logSheet.getRange('A1:O1').setValues([[
      'id', 'date', 'startTime', 'endTime', 'durationSeconds',
      'actualDurationSeconds', 'type', 'description', 'category',
      'workInterruptions', 'nonWorkInterruptions',
      'workInterruptionSeconds', 'nonWorkInterruptionSeconds',
      'completionStatus', 'pomodoroSetIndex'
    ]]);
  ...
  if (!intSheet) {
    ...
    intSheet.getRange('A1:H1').setValues([[
      'id', 'pomodoroId', 'type', 'startTime', 'endTime',
      'durationSeconds', 'category', 'note'
    ]]);
  ...
  if (!configSheet) {
    ...
    configSheet.getRange('A1:F1').setValues([[
      'patternName', 'workMinutes', 'shortBreakMinutes',
      'longBreakMinutes', 'pomodorosBeforeLongBreak', 'isActive'
    ]]);
