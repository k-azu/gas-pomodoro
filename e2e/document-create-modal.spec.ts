...
test("プロジェクト配下では案件か直属タスクを選択して作成する", async ({ page }) => {
  await gotoApp(page);
  await page.getByRole("button", { name: "タスク", exact: true }).click();
  ...
  await dialog.getByLabel("名前").fill("モーダルから作成した案件");
  const caseButton = dialog.getByRole("button", { name: "案件作成" });
  ...
  await expect(page.getByText("モーダルから作成した案件", { exact: true })).toBeVisible();
...
test("案件配下では種類選択なしでタスクを作成する", async ({ page }) => {
  await gotoApp(page);
  await page.getByRole("button", { name: "タスク", exact: true }).click();
  ...
  await dialog.getByLabel("タスク名").fill("案件直属の新規タスク");
