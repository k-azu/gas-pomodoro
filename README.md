...
- Google アカウント
- 文書（本文・名称・タグなど）を編集する場合は Web Locks API 対応ブラウザ

同一ブラウザ・同一オリジンにおける文書編集の複数タブ排他には Web Locks API を使用する。
非対応ブラウザでは同一文書を安全に保護できないため、本文とメタデータは読み取り専用になる。
本文はサーバー側でもrevision競合を検出するが、複数端末間のメタデータ更新はCAS対象外であり、
最後にサーバーで処理された更新が優先される。
並び順は文書単体のメタデータではなくコレクション状態として扱う。同一ブラウザ内では最新の並び順を
IndexedDBへ永続化してタブ間で共有し、コレクション単位のWeb Lockでサーバー送信を直列化する。
